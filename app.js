...
function toggleRadio() {
  ...
  if (radioOn) {
    ...
    btn.textContent = 'ON';
    // Seed with initial headlines so ticker isn't blank
...
function playerBotTrade(side, usd, source) {
  ...
  const amount = usd / price;
  const fee = usd * 0.001;
  const result = executeGameTrade('player', side, price, usd, amount, fee);
  if (result) {
...
function playerScalpTick() {
  ...
  } else if (bot.priceHistory.length >= 3) {
    const prev = bot.priceHistory[bot.priceHistory.length - 3];
    const dipPct = (prev - price) / prev * 100;
    if (dipPct >= 0.1 && dipPct <= 0.3 && GAME.player.balance >= tradeSize) {
...
function playerDipTick() {
  ...
  } else if (dipPct >= threshold && GAME.player.balance >= tradeSize) {
...
function playerGridTick() {
  ...
  if (bot.holding > 0 && price >= bot.upper) {
    const usd = bot.holding * price;
    if (playerBotTrade('sell', usd, 'Grid')) {
      bot.holding = 0;
      bot.trades++;
      updatePlayerBotStatus('grid', `🟢 Sold at upper (${bot.trades} trades)`);
    }
  } else if (bot.holding === 0 && price <= bot.lower && GAME.player.balance >= tradeSize) {
    if (playerBotTrade('buy', tradeSize, 'Grid')) {
      bot.holding = tradeSize / price;
      bot.trades++;
      updatePlayerBotStatus('grid', `🟢 Bought at lower (${bot.trades} trades)`);
    }
  }
}
...
function toggleNovaGameChat() {
  ...
  document.getElementById('nova-game-toggle').textContent = body.classList.contains('hidden') ? '▼' : '▲';
}
