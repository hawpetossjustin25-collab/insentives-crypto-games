...
const LIVE_ORDER_FLOW = {
  ...
  useRealData: localStorage.getItem('incentives-live-orderflow') === 'true' || false
};
...
function initLiveOrderFlow() {
  ...
  try {
    ...
    LIVE_ORDER_FLOW.ws.onopen = () => {
      ...
      const krakenPair = LIVE_ORDER_FLOW.currentSymbol === 'XETHZUSD' ? 'ETH/USD' :
                          LIVE_ORDER_FLOW.currentSymbol === 'XXBTZUSD' ? 'XBT/USD' :
                          LIVE_ORDER_FLOW.currentSymbol === 'SOLUSD' ? 'SOL/USD' :
                          'ETH/USD'; // fallback
