...
// ============================================================
// === GAME PANEL RESIZE HANDLE ================================
// ============================================================
function initGamePanelResize() {
  const divider = document.querySelector('.gp-divider');
  if (!divider) return;
  divider.classList.add('resize-handle-v');

  const playerPanel = document.querySelector('.gp-player');
  const botPanel = document.querySelector('.gp-bot');
  if (!playerPanel || !botPanel) return;

  let dragging = false, startX, startPlayerW, startBotW;

  divider.addEventListener('mousedown', (e) => {
    dragging = true;
    startX = e.clientX;
    startPlayerW = playerPanel.offsetWidth;
    startBotW = botPanel.offsetWidth;
    document.body.style.cursor = 'col-resize';
    document.body.style.userSelect = 'none';
    e.preventDefault();
  });
  ...
  document.addEventListener('mousemove', (e) => {
    ...
    playerPanel.style.width = Math.max(250, startPlayerW + dx) + 'px';
    botPanel.style.flex = 'none';
    botPanel.style.width = Math.max(250, startBotW - dx) + 'px';
  });
